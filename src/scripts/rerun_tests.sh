...
buildEnvironmentVariable() {
  ...
  echo "$envKey"_"$(echo "${sanitized_values[*]}" | awk '{gsub(/ /, "_"); print}')"
}
...
if echo "$WORKFLOW_RESPONSE" | jq empty 2>/dev/null; then
  ...
  if [[ "$WORKFLOW_TAG" =~ rerun ]]; then
    logger "SUCCESS" "Workflow is a rerun. Proceeding with env var operations..."
  else
    logger "WARNING" "Workflow is not a rerun (tag: $WORKFLOW_TAG). Skipping env var operations."
    exit 0
  fi
else
